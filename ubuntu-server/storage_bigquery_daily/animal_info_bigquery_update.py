...
def update_data(project_id, dataset_id, table_id, update_df):
    if len(update_df) > 0 :
        ...
        filter_condition = f"desertionNo IN UNNEST([{','.join(str(n) for n in desertion_nos)}])"
        ...
        for _, row in existing_data.iterrows():
            ...
            process_state = update_df.loc[update_df['desertionNo'] == desertion_no, 'processState'].values[0]
...
def process_data(bucket_name, file_name, project_id, dataset_id, table_id):
    ...
    # 업데이트해야 할 데이터 필터링
    update_df = df[df['desertionNo'].isin(existing_data.keys()) & (df['processState'] != df['desertionNo'].map(existing_data))].copy()
    update_df = update_df.rename(columns={'Unnamed: 0': 'int64_field_0'})

    ## 기존 빅쿼리에 들어있는 데이터 업데이트
    update_data(project_id, dataset_id, table_id, update_df[['desertionNo','processState']])
    ...
    new_df.to_csv(f'/home/ubuntu/openAPI/data/new_data_{(datetime.now() - relativedelta(days=1)).strftime("%Y%m%d")}.csv')
    update_df.to_csv(f'/home/ubuntu/openAPI/data/changed_data_{(datetime.now() - relativedelta(days=1)).strftime("%Y%m%d")}.csv')
    print(f'save the new_df file {(datetime.now() - relativedelta(days=1)).strftime("%Y%m%d")}.csv ')
    print(f'save the update_df file {(datetime.now() - relativedelta(days=1)).strftime("%Y%m%d")}.csv ')
