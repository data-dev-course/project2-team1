...
def date_range(start, end):
        ...
        dates = [(start + timedelta(days=i)).strftime("%Y%m%d") for i in range((end-start).days)]
        return dates
...
def extract_data_OpenAPI(**context):
    ...
    before_one_month = (datetime.now(timezone('Asia/Seoul')) - relativedelta(months=1)).strftime("%Y-%m-%d")
        ...
        for _ in range(MAX_RETRIES):
            try:
                ...
                if 'SERVICE_KEY_IS_NOT_REGISTERED_ERROR' in my_response.text:
                    raise Exception('공공데이터 OpenAPI Server error: SERVICE_KEY_IS_NOT_REGISTERED_ERROR')
                else:
                    df_dailydata = pd.DataFrame(json.loads(my_response.text)['response']['body']['items']['item']) # daily data json -> df
                    result_df = pd.concat([result_df, df_dailydata], ignore_index=True)
                    daily_data_cnt += df_dailydata.shape[0]
                    print(date, '성공 데이터 :', df_dailydata.shape[0] ,'/', '총 데이터 :', daily_data_cnt)
                    break
    ...
    LOCAL_PATH_NAME = os.path.join(os.environ['AIRFLOW_HOME'], 'data', 'strayanimal_30days_data', SAVE_NAME)
...
def upload_data_GCS(**context):
    SAVE_NAME, LOCAL_PATH_NAME = context['ti'].xcom_pull(task_ids='extract_data_OpenAPI')
