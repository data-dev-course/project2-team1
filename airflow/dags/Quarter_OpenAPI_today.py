...
def extract_data_OpenAPI(**context):
    ...
    for _ in range(MAX_RETRIES):
        try:
            ...
            if 'SERVICE_KEY_IS_NOT_REGISTERED_ERROR' in my_response.text:
                ...
                df_today_data = pd.DataFrame(json.loads(my_response.text)['response']['body']['items']['item']) # daily data json -> df
    ...
    LOCAL_PATH_NAME = os.path.join(os.environ['AIRFLOW_HOME'], 'data', 'strayanimal_today_data', SAVE_NAME)
