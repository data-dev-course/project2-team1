    ...
    for _ in range(MAX_RETRIES):
        try:
            ...
            if res.status_code != 200:
                print(res)
                raise Exception("GCP VM Server requests error, code: " + str(res.status_code))
                ...
                df_today_data = pd.DataFrame(res_json["response"]["body"]["items"]["item"])  # daily data json -> df
            ...
            if e.__class__ is requests.exceptions.ReadTimeout:
                ...
                print(today_HM, '|', res.status_code, "ConnectTimeout Error : " + str(e))
            else:
