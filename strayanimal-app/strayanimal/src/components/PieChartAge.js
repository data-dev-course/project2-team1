...
function PieChartAge(params) {
	...
	const { status, data } = useQuery(
		["strayanimal", "casePerAgeData"],
		async () => {
			const docSnap = await getDoc(
				doc(db, "strayanimal", "차트10_연령대_별_유기_발생_비율")
			);
	...
	return (
		<div className="horizontal align-center" style={{ width: "100%" }}>
			<canvas id="pie_chart_per_age" ref={chartRef} />
		</div>
	);
}
