...
function EndStateChart(params) {
	...
	const lt60dayChart = (dataset) => {
		...
		const lt60DayDataset = [
			low60Day.reduce(
				(partialSum, a) => partialSum + parseInt(a.count),
				0
			),
			above60Day.reduce(
				(partialSum, a) => partialSum + parseInt(a.count),
		...
		return {
			...
			data: {
				...
				datasets: [
					{
						label: "종료상태",
						data: subDatasetlt60Day.map((elem) =>
							parseInt(elem.count)
						),
			...
			const percent_kind = [
				{
					endState: "개",
					count: data[2]
						.filter((data) => data.kindCd === "개")
						.reduce((sum, a) => sum + parseInt(a.count), 0),
				},
				{
					endState: "고양이",
					count: data[2]
						.filter((data) => data.kindCd === "고양이")
						.reduce((sum, a) => sum + parseInt(a.count), 0),
				},
				{
					endState: "기타축종",
					count: data[2]
						.filter((data) => data.kindCd === "기타축종")
						.reduce((sum, a) => sum + parseInt(a.count), 0),
				},
			];
			if (params.type === "전체") {
				...
				setDoughnutData(
					dataconfig(data[2].filter((data) => data.kindCd === "개"))
				);
			} else if (params.type === "고양이") {
				setDoughnutData(
					dataconfig(
						data[2].filter((data) => data.kindCd === "고양이")
					)
				);
			} else {
				setDoughnutData(
					dataconfig(
						data[2].filter((data) => data.kindCd === "기타축종")
					)
				);
			}
		}
	...
	return (
		<div>
			<canvas id={params.type} ref={chartRef} />
		</div>
	);
}
