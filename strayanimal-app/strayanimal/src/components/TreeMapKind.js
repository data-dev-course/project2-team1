...
function TreeMapKind() {
  ...
  const { status, data } = useQuery(
    ["strayanimal", "animalkindmap"],
    async () => {
      const docSnap = await getDoc(
        doc(db, "strayanimal", "차트11_품종_견묘_등록_유기_비율")
      );
    ...
    <div id="treeWrapper">
      <div className="horizontal align-start" style={{ padding: "10px 0" }}>
        <button
          ...
          style={{
            backgroundColor:
              selectViewType === "staryCnt" ? "#FF5F15" : "#4E4E4E",
          }}
        >
          유기동물
        </button>
        <button
          ...
          style={{
            backgroundColor:
              selectViewType === "registerCnt" ? "#FF5F15" : "#4E4E4E",
          }}
        >
          등록동물
        </button>
