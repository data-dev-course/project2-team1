	...
	const printD3 = (datasets) => {
		...
		mapLayer
			...
			.style("fill", (d) => {
				...
				const matchingData = datasets.find(
					(item) => item.name === name
				);
		...
		const legendItems = legend
			...
			.attr(
				"transform",
				(d, i) => `translate(0, ${i * legendItemHeight + 10})`
			)
			.style("fill", "#Fff");
		...
		mapLayer.selectAll("path").on("mouseover", function (event, d) {
			...
			d3.select(this).style("fill", "#00B3A0");
